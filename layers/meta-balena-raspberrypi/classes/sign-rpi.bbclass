...
do_sign_rpi() {
    ...
    if [ "x${SIGN_RPI_KEY_ID}" = "x" ]; then
        bbfatal "RPI key ID must be defined"
    fi
    ...
    if CURL_CA_BUNDLE="${STAGING_DIR_NATIVE}/etc/ssl/certs/ca-certificates.crt" curl --retry 5 --silent --show-error --max-time ${_timeout} "${SIGN_API}/rpi/sign" -X POST -H "Content-Type: application/json" -H "X-API-Key: ${SIGN_API_KEY}" -d "@${REQUEST_FILE}" --output "${RESPONSE_FILE}"; then
